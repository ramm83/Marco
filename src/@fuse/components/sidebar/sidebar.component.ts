...
export class FuseSidebarComponent implements OnInit, OnDestroy {
    ...
    private _backdrops: HTMLElement[] = [];
    ...
    private _showBackdrop(): void {
        ...
        this._backdrops.push(this._backdrop);
        ...
        this._backdrop.addEventListener('click', () => {
            this.close();
            this.toggleFold(this._selectedItem);
        });
    ...
    close(): void {
        ...
        this._extensibleService.changeSelectedGroup(null);
        // Hide the backdrop
        this._hideBackdrop();
        // Enable the animations
        this._enableAnimations();
    ...
    onMouseEnter(): void {
        this._interfaceService.toggleShowOptionsSearchBox(false);
    ...
    onMouseLeave(): void {
        this._interfaceService.toggleShowOptionsSearchBox(true);
        // Only work if the auto trigger is enabled
        if (!this.foldedAutoTriggerOnHover) {
            return;
        }
     ...
     */
    toggleFold(selectedItem: any = this._selectedItem): void {
        if (this.folded || selectedItem !== this._selectedItem) {
            this._selectedItem = selectedItem;
            this.folded && this.isLockedOpen && this._showBackdrop();
            this.isLockedOpen  && this.unfold();
        } else {
            if (selectedItem) {
                this.fold();
                this._hideBackdrop();
